def generate_code(data):
    ...
    # Combine data and parity bits to form the code
    code = [d1, d2, d3, p1, p2]
    return code
...
def detect_error(code):
    ...
    # Compare calculated parity bits with expected parity bits
...
# Get input for data bits
data_bits = []
for i in range(3):
    bit = int(input(f"Enter bit {i + 1} (0 or 1): "))
    data_bits.append(bit)

# Generate code with parity bits
code = generate_code(data_bits)
print("Generated code:", code)

# Introduce an error (change a bit in C1)
error_location = int(input("Enter position to simulate error (1-5): "))
code[error_location - 1] = 1 - code[error_location - 1]  # introduce an error
